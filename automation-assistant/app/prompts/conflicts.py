...
def build_batch_analysis_prompt(
    ...
    return (
        ...
        "Sensitivity rules (be conservative):\n"
        "- Only flag issues you can directly infer from the automation text or "
        "invalid entities. Do NOT guess missing behavior.\n"
        "- It is common to split ON and OFF into separate automations. Do NOT "
        "flag 'never turns off' or 'missing off' unless you can confirm no other "
        "automation handles the off action and the automation itself implies an "
        "off action should occur (e.g., an explicit delay/wait with no follow-up).\n"
        "- Treat complementary on/off automations with different triggers "
        "(e.g., motion on vs no-motion for 5m) as NORMAL, not a conflict.\n"
        "- Only report conflicts when opposing actions target the same entity AND "
        "the triggers can overlap in time.\n"
        "- If unsure, set status=ok and do not include an issue.\n\n"
